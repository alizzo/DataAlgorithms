...
public class Assignment5_Problem1 {	
	...
	//condition 1: the values are even
			...
			if(i == 0 && s1[i] == s1[i+1]){
				...
				//	System.out.print(i+ "i ");		//test to find position index
				//	System.out.print(j+ "j ");		//test to find position index
				...
				if(( j == 0 && s2[j] == s2[j+1]) || (j >= 1 && s2[j] == s2[j-1] ) ) {
					j++;
	...
	//		System.out.print(j+ "j ");
				...
				if(( i == 0 && s1[i] == s1[i+1]) || (i >= 1 && s1[i] == s1[i-1] ) ) {
					i++;
					//System.out.print(i+ "i ");
					if(i> s1.length-1 ){           
					//System.out.print(s2.length-1 );
